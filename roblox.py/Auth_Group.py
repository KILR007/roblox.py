...
from .utils import send_request,request
from .exceptions import GroupNotFound


class GroupAuth:
    def __init__(self,groupID:int,cookies=None):
    ...
    async def pay(self, user_id: int, amount: int):
        ...
        r = await request(url=f'https://groups.roblox.com/v1/groups/{self.Target_grp_id}/payouts', data=data,method="post",cookies=self.cookies)
        return r
    async def change_description(self,desc=None):
        data = {"description": desc}
        e = await request(url=f"https://groups.roblox.com/v1/groups/{self.Target_grp_id}/description",method='patch',data=data,cookies=self.cookies)
        return e
